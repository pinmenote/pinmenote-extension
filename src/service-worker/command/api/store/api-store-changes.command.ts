...
import { HashChange } from './api-store.model';
import { ICommand } from '../../../../common/model/shared/common.dto';
import { fnConsoleLog } from '../../../../common/fn/fn-console';

export class ApiStoreChangesCommand
  extends ApiCallBase
  implements ICommand<Promise<{ data: HashChange[] } | undefined>>
{
  async execute(): Promise<{ data: HashChange[] } | undefined> {
    await this.initTokenData();
    if (!this.storeUrl) return;
    try {
      const resp = await FetchService.fetch<{ data: HashChange[] }>(
