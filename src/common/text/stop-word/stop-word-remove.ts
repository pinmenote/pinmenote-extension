...
import stopWords from '../../../vendor/text/stop-word/stopwords-iso.json';
  ...
  static execute(iso2lang: string, inputWords: string[]): string[] {
    ...
    if (profiles.includes(iso2lang)) {
      ...
      for (let i = 0; i < inputWords.length; i++) {
        ...
        if (!languageProfile.includes(word)) {
          out.push(word);
        }
      }
      // console.log('StopWordRemove', iso2lang, ' input', inputWords.length, 'output', out.length);
      return out;
    }
